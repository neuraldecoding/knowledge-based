# knowledge-based

Berisi pengetahuan dasar dan perkembangan dari metode-metode terkini. 

Dasar-dasar dari [Machine Learning](https://mml-book.github.io/book/mml-book.pdf)

<img width="1280" height="714" alt="image" src="https://github.com/user-attachments/assets/e5291cd4-0f8d-44d3-b4f9-12be4192e0b8" />

## Struktur

- [Video Transformer](https://youtu.be/zxQyTK8quyY?si=2Aq8OHtJqQsCzB8G)
- [Transformer](./transformer/)
- [Tiny Recursife Model](./trm/)
- [State Space Model](./ssm/)
- [Dasar Mamba](./mamba/)
- [Perbandingan Mamba](https://newsletter.maartengrootendorst.com/p/a-visual-guide-to-mamba-and-state)
- [Contrastive Learning](./cl/)
- [Contrastive LanguageImage Pre-training (CLIP)](./clip/)
- [InfoNCE Loss](./infonce)
- [Diffusion Probabilistic Models](./dpm/)
